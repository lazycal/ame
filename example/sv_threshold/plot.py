...
path = [
    ...
    ('Paired Sampling', f'{root}/PS/run-*/output.pkl'),
...
names = [r'AME $\epsilon=0.1$', r'AME $\epsilon=0.01$',
         'Compr. Sensing', 'KernelSHAP (L1)', 'KernelSHAP', 'Paired Sampling']
ax, handles, labels = plot(df, figsize=(6, 4), names=names)
ax.legend(handles[:], labels[:], handletextpad=0, columnspacing=0,
          ncol=1)
...
path = [
    ...
    (r'AME $\epsilon=0.1$', f'{root}/TU0.1/run-*/output.pkl'),
    (r'AME $\epsilon=0.01$', f'{root}/TU0.01/run-*/output.pkl'),
    ('Group Testing\n[Jia et al.]', f'{root}/GTdft/run-*/output.pkl'),
]
df = load_data(path)
ax, _, _ = plot(df, use_order=False, exclude=[], figsize=(7, 5))
plt.tight_layout(pad=0)
